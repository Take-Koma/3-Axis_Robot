...
def main():
    ...
    ckpt_file_path = Path(".../results/Padim/Test_Product/latest/weights/lightning/model.ckpt").resolve() 
    ...
    # 3. モデルの「空の枠組み」を用意（学習時と同じPadimを指定）
    model = Padim()
    ...
    try:
        exported_path = engine.export(
            ...
            input_size=(256, 256)   # ここで入力サイズを指定して固定化します（TwinCATは可変サイズを許さないため）
        )
        print("=== エクスポート大成功！ ===")
